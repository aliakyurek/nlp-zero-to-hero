# Language_translation_with_transformers
import base
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        weighted_normalized_src = self.nn_mhsa_layer_norm(
            src + self.nn_dropout(weighted_src))  # [batch size, src len, hid dim]
        # Operation:Feed forward
        pwffed_weighted_normalized_src = self.nn_positionwise_feedforward(
            weighted_normalized_src)  # [batch size, src len, hid dim]
        # Operation:Second Add&Norm
        final_out = self.nn_pwff_layer_norm(
            weighted_normalized_src + self.nn_dropout(pwffed_weighted_normalized_src))  # [batch size, src len, hid dim]
        return final_out
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, hid_dim, n_heads, dropout):
        ...
        # we could also combine q,k,v into one linear layer, but it'd be more complex
    ...
    def forward(self, query, key, value, mask=None):
        ...
        q = q.reshape(batch_size, q_len, self.n_heads, self.per_head_dim).permute(0, 2, 1, 3)
        # [batch size, n heads, query len, per head dim]
        k = k.reshape(batch_size, k_len, self.n_heads, self.per_head_dim).permute(0, 2, 1, 3)
        # [batch size, n heads, key len, per head dim]
        v = v.reshape(batch_size, v_len, self.n_heads, self.per_head_dim).permute(0, 2, 1, 3)
        # [batch size, n heads, value len, per head dim]
        ...
        energy = torch.matmul(q, k.transpose(-2, -1)) / self.scale.to(
            q.device)  # energy = [batch size, n heads, query len, key len]

        if mask is not None:
            energy = energy.masked_fill(~mask, float('-inf'))  # n_heads and query len dimensions of mask broadcasted

        attention = torch.softmax(energy, dim=-1)
        # [batch size, n heads, query len, key len] Rows summed to one in the last dimension
...
class Decoder(nn.Module):
    ...
    def forward(self, trg, trg_mask, enc_src, src_mask):
        ...
        for layer in self.nn_layers:
            trg, attention = layer(trg, trg_mask, enc_src,
                                   src_mask)  # attention = [batch size, n heads, trg len, src len]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg, trg_mask, enc_src, src_mask):
        ...
        # Operation:First Add&Norm
        weighted_normalized_trg = self.nn_mhsa_layer_norm(
            trg + self.nn_dropout(weighted_trg))  # [batch size, trg len, hid dim]
        # Operation:Multi-head cross attention
        cross_weighted_trg, attention = self.nn_mh_cross_attention(query=weighted_normalized_trg, key=enc_src,
                                                                   value=enc_src, mask=src_mask)
        # Operation:Second Add&Norm
        cross_weighted_normalized_trg = self.nn_mhea_layer_norm(
            weighted_normalized_trg + self.nn_dropout(cross_weighted_trg))  # [batch size, trg len, hid dim]
        # Operation:Feed forward
        ...
        final_out = self.nn_pwff_layer_norm(cross_weighted_normalized_trg +
                                            self.nn_dropout(
                                                pwffed_cross_weighted_normalized_trg))  # [batch size, trg len, hid dim]
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg=None):
        ...
        for _ in range(trg_len):
            ...
            output, attention = self.m_decoder(trg, trg_mask, enc_src, src_mask)
            # output [1,n,5000] during inference, else [batch size, trg len, output_dim]
